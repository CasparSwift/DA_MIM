...
def get_results(method='MIM', task='AE'):
	...
	for s, t in domain_pair:
		...
		for name in names:
			...
			f1_aes_cur_run = []
			if task =='ABSA':
				loggings = ['1113_ABSA', '1113_ABSA_2', 'ABSA_3', 'ABSA_4']
			...
			for logging in loggings:
				...
				print(s+t, logging, f1s, '%.2f' % (micro_f1))
				# print(f'f1: {f1s} macro: {macro_f1:.2f} micro: {micro_f1:.2f}')
				# print(f'f1_ae: {f1_aes} macro: {macro_f1_ae:.2f} micro: {micro_f1_ae:.2f}')
				absa_result[s+t] = absa_result.get(s+t, []) + [(logging, f1s, '%.2f' % (micro_f1))]
				ae_result[s+t] = ae_result.get(s+t, []) + [(logging, f1_aes, '%.2f' % (micro_f1_ae))]
	...
	return absa_result, ae_result


if __name__ == '__main__':
	r1, r2 = get_results(method='MIM', task='ABSA')
	for k, v in r1.items():
		print(k, v)
